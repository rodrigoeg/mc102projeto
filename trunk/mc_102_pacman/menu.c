        ...
        if (!(mouse_b & 1)) {
            *botao_mouse_pressionado = FALSE;
            if ((mouse_x > 230 && mouse_x < 390) && (mouse_y > 280 && mouse_y < 305)) {
                play_sample(som_menu, 500, 50, 2000, 0);
                *estado_jogo = JOGO;
                ...
                *fase_atual = 0;
                *score = 0;
                atualiza_tela(buffer, 0, fases_cenario, namcap, texturas, ultima_movimentacao, sheet);
        ...
        if (!(mouse_b & 1)) {
            *botao_mouse_pressionado = FALSE;
            ...
            if ((mouse_x > 70 && mouse_x < 440) && (mouse_y > 195 && mouse_y < 240)) {
                ...
                *fase_atual = 1;
                *score = 0;
                atualiza_tela(buffer, 1, fases_cenario, namcap, texturas, ultima_movimentacao, sheet);
                draw_sprite(buffer, score_back, 0, 0);
                blit(buffer, screen, 0, 0, 0, 0, SCREEN_W, SCREEN_H);
                stop_midi();
                play_sample(intro, 500, 50, 1000, 0);
            ...
            if ((mouse_x > 70 && mouse_x < 390) && (mouse_y > 270 && mouse_y < 315)) {
                ...
                *fase_atual = 2;
                *score = 0;
                atualiza_tela(buffer, 2, fases_cenario, namcap, texturas, ultima_movimentacao, sheet);
                draw_sprite(buffer, score_back, 0, 0);
                blit(buffer, screen, 0, 0, 0, 0, SCREEN_W, SCREEN_H);
                stop_midi();
                play_sample(intro, 500, 50, 1000, 0);
...
void final_jogo(BITMAP *buffer, BITMAP *final, BITMAP *numeros, int score) {
    ...
    while (digitos > 0) {
        if (digitos > 1) {
            masked_blit(numeros, buffer, 0, (int) (score / pow(10, digitos - 1))*30, (digitos_inicial - digitos)*20 + 285, 185, numeros->w, 27);
